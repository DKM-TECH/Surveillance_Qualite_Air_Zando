...
def get_mesures():
    ...
    # -------------------------
    # Suppression valeurs nulles extrêmes
    # -------------------------
...
async def capteur_post(data: dict):
    ...
    try:
        ...
        db.collection("mesures").add(mesure)
...
def apriori_page(request: Request):
    ...
    if not df.empty:
        ...
        df_latest = df.sort_values(
            by="timestamp",
            ascending=False
        ).head(1)

        dernieres_mesures = {
            k: float(df_latest.iloc[0][k])
            for k in SEUILS
        }
    ...
    if not df.empty and len(df) >= 5:
        ...
        df_polluants = df[list(SEUILS.keys())]

        # Binarisation
        df_bin = df_polluants.gt(
            pd.Series(SEUILS)
        ).astype(int)
